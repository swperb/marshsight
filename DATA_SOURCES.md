...
| Hydrography (rivers, creeks, lakes, ponds) | USGS National Hydrography Dataset (NHD) | Public domain | The National Map ArcGIS REST, paged so a region captures the full network down to small creeks and ponds |
...
| Hunting units (game zones / WMAs / GMUs) | State wildlife agencies (per state) + USFWS national | Public domain / public records | Direct ArcGIS REST per state, 47 of 50 states (deer zones / WMAs / game-management units / state game lands depending on what each agency publishes), keyed by state in HuntingUnitsService.registry. National overlay everywhere: USFWS National Wildlife Refuge hunt units (huntable units only). AL uses a hosted public-record WMA copy (official DCNR server unreachable). No open state layer found yet for IL, SD, and DE (DE official server returns 500); all three still get the federal refuge overlay. |
