...
| Hunting units (game zones / WMAs / GMUs) | State wildlife agencies (per state) + USFWS national | Public domain / public records | Direct ArcGIS REST per state, ~38 states (deer zones / WMAs / game-management units / state game lands depending on what each agency publishes), keyed by state in HuntingUnitsService.registry. National overlay everywhere: USFWS National Wildlife Refuge hunt units (huntable units only). AL uses a hosted public-record WMA copy (official DCNR server unreachable). No open state layer found yet for IL and SD. |
